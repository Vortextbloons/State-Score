...
export type Metric={id:number;categoryId:number;slug:string;name:string;description?:string;unit?:string;higherIsBetter:boolean;normalizationMethod:string;defaultWeight:number;active?:boolean};
export type MetricValueQuality={reportingCoverage?:number;participatingAgencies?:number;populationCovered?:number;dataRevision?:string;scoringEligible:boolean;exclusionReason?:string};
export type MetricValue={id:number;stateId:number;metricId:number;year:number;value:number;sourceRecordId?:string;importId?:number;quality?:MetricValueQuality};
...
export type StateScore={stateId:number;overallScore:number;completeness:number;calculatedAt?:string;calculationVersion?:string;categories:CategoryScore[]};
export type Scoreboard={profileId:number;year:number;asOfYear:number;calculationVersion:string;scores:StateScore[]};
